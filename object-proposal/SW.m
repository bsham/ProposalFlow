function [ candidates, scores ] = SW( im, num_samples_selected)
...
num_samples=3*num_samples_selected;
...
[candidates, score] = remove_boundary_box(im, candidates, scores);

if size(candidates, 1) > num_samples_selected
    idx = randi(size(candidates, 1),[1, num_samples_selected]);
...
candidates(:,3) = min(size(im, 2), candidates(:,3));
candidates(:,4) = min(size(im, 1), candidates(:,4));
